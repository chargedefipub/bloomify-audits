...
contract StrategyCakeLP is FeeManager, GasThrottler {
    ...
    function chargeFees(address callFeeRecipient) internal {
        uint256 toNative = IERC20(output).balanceOf(address(this)).mul(425).div(
            10000
        );
        ...
        uint256 bloomifyFeeAmount = nativeBal.mul(bloomifyFee).div(MAX_FEE);
        IERC20(native).safeTransfer(bloomifyFeeRecipient, bloomifyFeeAmount);
    ...
    function callReward() public view returns (uint256) {
        ...
        if (_outputBal > 0) {
            uint256[] memory _amountOut = IUniswapRouterETH(unirouter)
                .getAmountsOut(_outputBal, outputToNativeRoute);
